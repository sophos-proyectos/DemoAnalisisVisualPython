# -*- coding: utf-8 -*-
"""
Created on Wed Sep 16 20:49:40 2020

@author: Usuario
"""

# importo librerias
import cv2
from skimage.feature import hog
import numpy as np
import joblib

# cargo modelo
clf =  joblib.load('Modelo.joblib')


# leo imagen
im = cv2.imread("placa_1.jpg")


# escalka de grises
im_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
im_gray = cv2.GaussianBlur(im_gray, (5, 5), 0)

# binario 
ret, im_th = cv2.threshold(im_gray, 90, 255, cv2.THRESH_BINARY_INV)

# busco figuras en la imagen
ctrs, hier = cv2.findContours(im_th.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

# cuentos figuras encontradas
rects = [cv2.boundingRect(ctr) for ctr in ctrs]



for rect in rects:
    # dibujo rectangulos
    cv2.rectangle(im, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0, 255, 0), 3) 

    leng = int(rect[3]*0.8)
    pt1 = int(rect[1] + rect[3] //2.5 - leng // 2)
    pt2 = int(rect[0] + rect[2] // 2.5 - leng // 2)
    roi = im_th[pt1-10:pt1+leng+200, pt2+50:pt2+leng-10]
    #roi = im_th[1105:169+50, 50:169+316]
    # Resize the image
    
    #roi = cv2.resize(roi, (28, 28), interpolation=cv2.INTER_AREA)
    
    #cv2.imshow("Resulting Image with Rectangular ROIs", roi)
    #cv2.waitKey()
    
    
    roi = cv2.resize(roi, (28, 28), interpolation=cv2.INTER_AREA)
    roi = cv2.dilate(roi, (3, 3))
    
    roi_hog_fd = hog(roi, orientations=9, pixels_per_cell=(14, 14), cells_per_block=(1, 1))
    nbr = clf.predict(np.array([roi_hog_fd], 'float64'))
    cv2.putText(im, str(int(nbr[0])), (rect[0], rect[1]),cv2.FONT_HERSHEY_DUPLEX, 2, (0, 0, 255), 3)


    
cv2.imshow("Resulting Image with Rectangular ROIs", im)
cv2.waitKey()

